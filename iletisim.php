...
if ($_POST) { // Sayfada post olup olmadığını kontrol ediyoruz.
	...
	$email=$_POST['email'];
	$konu = $_POST['konu']; // Sayfa yenilendikten sonra post edilen değerleri değişkenlere atıyoruz
	$mesaj = $_POST['mesaj'];

	if ($email<>"" && $konu<>"") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz. başka kontrollerde yapabilirsiniz.
		
		if ($baglanti->query("INSERT INTO mesajlar (email,konu,mesaj) VALUES ('$email','$konu','$mesaj')")) // Veri ekleme sorgumuzu yazıyoruz.
		{
			echo "Mesajınız Admine Gönderildi"; // Eğer veri eklendiyse eklendi yazmasını sağlıyoruz.
